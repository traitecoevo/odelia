...
prepare_leaf_thermal_sourcecpp <- function(rebuild = TRUE, verbose = TRUE) {
  # `rebuild`: force recompilation when C++ has changed.
  # `verbose`: show compiler commands/messages for debugging.

  # Preload odelia shared library with global symbol visibility.
  # This avoids macOS flat-namespace linker errors for XAD symbols
  # (e.g., Tape<double,1>::active_tape_) when loading sourceCpp .so files.
  odelia_so <- file.path(
    system.file("libs", .Platform$r_arch, package = "odelia"),
    paste0("odelia", .Platform$dynlib.ext)
  )
  if (!file.exists(odelia_so)) {
  ...
  # Compile the generic interface first because it provides Drivers_* and
  # OdeControl_* entry points used by the leaf-specific R6 wrappers.
  Rcpp::sourceCpp("../../../src/ode_interface.cpp",
    verbose = verbose, rebuild = rebuild
  )

  # Compile the leaf-thermal interface (model-specific exported functions).
  Rcpp::sourceCpp("src/leaf_thermal_interface.cpp",
    verbose = verbose, rebuild = rebuild
  )
}

prepare_leaf_thermal_sourcecpp(rebuild = TRUE, verbose = TRUE)

# Load R6 wrappers after C++ symbols exist in the R session.
...
runner2 <- LeafThermalSolver$new(lz2$ptr, ctrl$ptr, drivers2$ptr)
