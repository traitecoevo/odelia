LeafThermalSolver <- R6::R6Class(
  ...
  public = list(
    ...
    state = function() {
      LeafSolver_state(self$ptr)
    },
    set_state = function(y, time) {
      LeafSolver_set_state(self$ptr, y, time)
      invisible(self)
    },
    times = function() {
      LeafSolver_times(self$ptr)
    },
    advance_adaptive = function(times) {
      LeafSolver_advance_adaptive(self$ptr, times)
      invisible(self)
    },
    ...
    reset = function() {
      LeafSolver_reset(self$ptr)
      invisible(self)
    },
    collect = function(value) {
    if (missing(value)) {
      LeafSolver_get_collect(self$ptr)
    } else {
      LeafSolver_set_collect(self$ptr, value)
    }
    },
    history_size = function() {
      LeafSolver_get_history_size(self$ptr)
    },
    history_step = function(i) {
      LeafSolver_get_history_step(self$ptr, i)
    },
    history = function() {
